...
async def do_ban_action(chat_id: int, user_id: int) -> str:
    try:
        ...
        return f'{E_CHECK} Заблокирован, сообщения удалены'
    except TelegramBadRequest as e:
        return f'{E_CROSS} {e.message}'

async def chat_mod_check(msg: Message) -> Optional[dict]:
    if msg.chat.type not in ("group", "supergroup"):
        await msg.answer(f'{E_CROSS} Команда только для групп.')
        return None
    result = await find_team_for_chat(msg.chat.id, msg.from_user.id, msg.from_user.username)
    if not result:
        await msg.answer(f'{E_CROSS} Группа не привязана или у вас нет прав модератора.')
    return result
