...
async def help_handler(msg: Message):
    await msg.answer(
        ...
        f'Работают в группах, привязанных к команде.\n\n'

        f'{E_LOG} <b>/log</b> — выдать лог (3 лога = варн)\n'
        f'<code>/log @user спам</code>\n'
        f'<code>/log спам</code> (ответом на сообщение)\n\n'

        f'{E_WARN} <b>/warn</b> — выдать варн (3 варна = бан)\n'
        f'<code>/warn @user оскорбление</code>\n'
        f'<code>/warn оскорбление</code> (ответом)\n\n'

        f'{E_BAN} <b>/ban</b> — заблокировать\n'
        f'<code>/ban @user причина</code>\n'
        f'<code>/ban причина</code> (ответом)\n\n'

        f'{E_UNLOCK} <b>/unlog</b> — снять последний лог\n'
        f'<code>/unlog @user</code>\n\n'

        f'{E_UNLOCK} <b>/unwarn</b> — снять последний варн\n'
        f'<code>/unwarn @user</code>\n\n'

        f'{E_UNLOCK} <b>/unban</b> — разбанить\n'
        f'<code>/unban @user</code>\n\n'

        f'{E_CHART} <b>/stats</b> — статистика нарушений\n'
        f'<code>/stats @user</code> или ответом',
        reply_markup=main_menu_kb()
    )
...
async def add_mod_username(msg: Message, state: FSMContext):
    ...
    await state.clear()
    status = f'{E_CHECK} Активен' if user_id else f'⏳ Активируется после /start'
    await msg.answer(
...
async def apply_punishment(msg: Message, ptype: str, team_id: int,
    ...
    if target_user_id is None:
        await msg.answer(
            f'{E_CROSS} @{target_username} не найден. Нужен /start.'
        )
        return
    ...
    if ptype == "log":
        ...
        if logs % 3 == 0:
            ...
            text += f'\n\n{E_WARN} <b>3 лога → варн!</b> ({warns_new}/3)'
            if warns_new % 3 == 0:
...
async def cmd_log(msg: Message):
    ...
    if not t:
        await msg.answer(f'{E_CROSS} <code>/log @user</code> или ответом')
        return
...
async def cmd_warn(msg: Message):
    ...
    t = await get_target(msg)
    if not t:
        await msg.answer(f'{E_CROSS} <code>/warn @user</code> или ответом')
        return
...
async def cmd_ban(msg: Message):
    ...
    t = await get_target(msg)
    if not t:
        await msg.answer(f'{E_CROSS} <code>/ban @user</code> или ответом')
        return
...
async def cmd_unlog(msg: Message):
    ...
    if not t or t[0] is None:
        await msg.answer(f'{E_CROSS} <code>/unlog @user</code>')
        return
    uid, uname = t
    ...
    if row:
        ...
        await msg.answer(f'{E_UNLOCK} Лог снят с {uname_str}. Активных: <b>{remaining}</b>')
    else:
        await msg.answer(f'{E_CROSS} Нет активных логов.')
...
async def cmd_unwarn(msg: Message):
    ...
    t = await get_target(msg)
    if not t or t[0] is None:
        await msg.answer(f'{E_CROSS} <code>/unwarn @user</code>')
        return
    uid, uname = t
    uname_str = f"@{uname}" if uname else str(uid)
    async with db_pool.acquire() as conn:
    ...
    if row:
        ...
        await msg.answer(f'{E_UNLOCK} Варн снят с {uname_str}. Активных: <b>{remaining}</b>')
    else:
        await msg.answer(f'{E_CROSS} Нет активных варнов.')
...
async def cmd_unban(msg: Message):
    ...
    t = await get_target(msg)
    if not t or t[0] is None:
        await msg.answer(f'{E_CROSS} <code>/unban @user</code>')
        return
    uid, uname = t
    uname_str = f"@{uname}" if uname else str(uid)
    try:
        ...
        await msg.answer(f'{E_CROSS} {e.message}')
...
async def cmd_stats(msg: Message):
    ...
    t = await get_target(msg)
    if not t or t[0] is None:
        await msg.answer(f'{E_CROSS} <code>/stats @user</code> или ответом')
        return
    uid, uname = t
    uname_str = f"@{uname}" if uname else str(uid)
    async with db_pool.acquire() as conn:
