...
async def add_group_start(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text(
        f'{E_PENCIL} Введите <b>username</b> (пример: @mygroup) или <b>chat ID</b> группы.\n\n'
        f'{E_INFO} Бот должен быть админом в группе с правом ограничивать участников и удалять сообщения.',
...
async def add_group_username(msg: Message, state: FSMContext):
    ...
    # Определяем: chat_id (число) или username
    chat = None
    username = None
    raw_clean = raw.lstrip("@")

    # Пробуем как chat ID (число, может быть отрицательным)
    try:
        chat_id = int(raw_clean)
        try:
            chat = await bot.get_chat(chat_id)
        except Exception:
            await msg.answer(f'{E_CROSS} Не удалось найти чат с ID <code>{chat_id}</code>. Бот должен быть в группе.')
            return
    except ValueError:
        # Не число — пробуем как username
        username = raw_clean
        try:
    ...
    final_username = username or chat.username
    ...
    async with db_pool.acquire() as conn:
        ...
        await conn.execute(
            ...
            team_id, chat.id, final_username, final_title
        )

    await state.clear()
    identifier = f"@{final_username}" if final_username else f"ID <code>{chat.id}</code>"
    await msg.answer(
        f'{E_CHECK} Группа <b>{final_title}</b> ({identifier}) добавлена!',
        reply_markup=main_menu_kb()
    )
