...
async def upsert_user(user_id: int, username: Optional[str], full_name: str):
    async with db_pool.acquire() as conn:
        ...
        if username:
            # Привязываем user_id к модераторским записям если человек был добавлен по username заранее
            await conn.execute(
...
async def check_bot_permissions(chat_id: int) -> tuple[bool, str]:
    try:
        ...
        if not getattr(member, "can_restrict_members", False):
            return False, "Боту нужно право <b>ограничивать участников</b>."
        return True, ""
...
async def help_handler(msg: Message):
    await msg.answer(
        f'{E_HELP} <b>Команды модерации</b>\n'
        f'Работают в группах, привязанных к вашей команде.\n\n'

        f'{E_LOG} <b>Логи</b> (3 лога = 1 варн)\n'
        f'<code>/log @user [комментарий]</code>\n'
        f'<code>/log [комментарий]</code> — ответом на сообщение\n'
        f'<code>/unlog @user</code> — снять последний лог\n\n'

        f'{E_WARN} <b>Варны</b> (3 варна = бан)\n'
        f'<code>/warn @user [комментарий]</code>\n'
        f'<code>/warn [комментарий]</code> — ответом на сообщение\n'
        f'<code>/unwarn @user</code> — снять последний варн\n\n'

        f'{E_BAN} <b>Баны</b>\n'
        f'<code>/ban @user [комментарий]</code>\n'
        f'<code>/ban [комментарий]</code> — ответом на сообщение\n'
        f'<code>/unban @user</code> — разбанить пользователя\n\n'

        f'{E_CHART} <b>Статистика</b>\n'
        f'<code>/stats @user</code> — статистика нарушений\n'
        f'<code>/stats</code> — ответом на сообщение\n\n'

        f'{E_INFO} Все команды используют только <b>модераторы и владелец</b> команды.',
        reply_markup=main_menu_kb()
    )
...
async def add_group_start(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text(
        ...
        f'{E_INFO} Бот должен быть добавлен в группу как администратор '
        f'с правом <b>ограничивать участников</b> и <b>писать сообщения</b>.',
...
async def add_group_username(msg: Message, state: FSMContext):
    ...
    try:
        ...
        await msg.answer(
            ...
            f'Убедитесь, что username указан верно и бот добавлен в группу.'
        )
        return
    ...
    if not ok:
        await msg.answer(
            f'{E_CROSS} <b>Недостаточно прав бота:</b>\n{err}\n\n'
            f'{E_INFO} Выдайте боту права администратора с разрешением ограничивать участников.'
        )
        return
    ...
    await state.clear()
    await msg.answer(
        f'{E_CHECK} Группа <b>{chat.title}</b> (@{username}) успешно добавлена!',
        reply_markup=main_menu_kb()
    )
...
async def mods_list(cb: CallbackQuery):
    ...
    hint = f'\n{E_INFO} ⏳ — ещё не писал боту /start' if has_pending else ""
    await cb.message.edit_text(
...
async def mod_info_cb(cb: CallbackQuery):
    ...
    if not m:
        await cb.answer("Модератор не найден.", show_alert=True)
        return
    status = (f'{E_CHECK} Активен (ID: <code>{m["user_id"]}</code>)'
              if m["user_id"] else '⏳ Ещё не писал боту /start')
    await cb.message.edit_text(
...
async def add_mod_start(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text(
        ...
        f'{E_INFO} Пользователь <b>не обязан</b> заранее писать боту — '
        f'права будут активны сразу, а user_id привяжется автоматически когда он напишет /start.',
        reply_markup=back_kb(f"mods:{team_id}")
    )
    await cb.answer()
...
async def add_mod_username(msg: Message, state: FSMContext):
    ...
    async with db_pool.acquire() as conn:
        # Нельзя добавить владельца как модератора
        ...
        if owner_username and owner_username.lower() == username_lower:
            await msg.answer(f'{E_CROSS} Владелец команды не может быть модератором.')
            await state.clear()
            return
        ...
        if existing:
            await msg.answer(f'{E_CROSS} @{raw} уже является модератором.')
            await state.clear()
            return

        # Пробуем найти user_id если он уже писал боту
    ...
    await state.clear()
    if user_id:
        status_text = f'{E_CHECK} Права активны (пользователь уже зарегистрирован в боте)'
    else:
        status_text = f'⏳ Права активируются когда @{raw} напишет боту /start'
    await msg.answer(
        f'{E_CHECK} @{raw} добавлен как модератор!\n'
        f'{E_INFO} {status_text}',
        reply_markup=main_menu_kb()
    )
...
async def get_target(msg: Message) -> Optional[tuple]:
    ...
    return None, username  # user_id неизвестен — вернём None
...
async def do_ban_action(chat_id: int, user_id: int) -> str:
    try:
        ...
        return f'{E_CHECK} Заблокирован'
    except TelegramBadRequest as e:
        return f'{E_CROSS} {e.message}'

async def chat_mod_check(msg: Message) -> Optional[dict]:
    if msg.chat.type not in ("group", "supergroup"):
        return None
    return await find_team_for_chat(msg.chat.id, msg.from_user.id, msg.from_user.username)
...
async def apply_punishment(msg: Message, ptype: str, team_id: int,
    ...
    if target_user_id is None:
        await msg.answer(
            f'{E_CROSS} Не удалось найти пользователя @{target_username} в системе.\n'
            f'{E_INFO} Пользователь должен хотя бы раз написать боту /start.'
        )
        return
    ...
    if ptype == "log":
        ...
        if logs % 3 == 0:
            ...
            text += f'\n\n{E_WARN} <b>3 лога → варн!</b> Варнов: <b>{warns_new}/3</b>'
            if warns_new % 3 == 0:
                ban_res = await do_ban_action(chat_id, target_user_id)
                async with db_pool.acquire() as conn:
                    await conn.execute(
                        """INSERT INTO punishments(team_id, chat_id, target_user_id, target_username, mod_user_id, type, comment)
                           VALUES($1,$2,$3,$4,$5,'ban','Автоматически за 3 варна')""",
                        team_id, chat_id, target_user_id, target_username, msg.from_user.id
                    )
                text += f'\n{E_BAN} <b>3 варна → автобан!</b> {ban_res}'
        await msg.answer(text)

    elif ptype == "warn":
        warns = await count_active(team_id, chat_id, target_user_id, "warn")
        text = (
            f'{E_WARN} <b>Варн выдан</b>\n'
            f'{E_TAG} Пользователь: {target_name}\n'
            f'{E_PROFILE} Модератор: {mod_name}\n'
...
async def cmd_log(msg: Message):
    ...
    if not t:
        await msg.answer(f'{E_CROSS} Укажите пользователя: ответом или <code>/log @username</code>')
        return
...
async def cmd_warn(msg: Message):
    ...
    t = await get_target(msg)
    if not t:
        await msg.answer(f'{E_CROSS} Укажите пользователя: ответом или <code>/warn @username</code>')
        return
...
async def cmd_ban(msg: Message):
    ...
    t = await get_target(msg)
    if not t:
        await msg.answer(f'{E_CROSS} Укажите пользователя: ответом или <code>/ban @username</code>')
        return
...
async def cmd_unlog(msg: Message):
    ...
    if row:
        ...
        await msg.answer(f'{E_UNLOCK} Лог снят с {uname_str}.\n{E_CHART} Активных логов: <b>{remaining}</b>')
    else:
        await msg.answer(f'{E_CROSS} У {uname_str} нет активных логов.')
...
async def cmd_unwarn(msg: Message):
    ...
    if row:
        ...
        await msg.answer(f'{E_UNLOCK} Варн снят с {uname_str}.\n{E_CHART} Активных варнов: <b>{remaining}</b>')
    else:
        await msg.answer(f'{E_CROSS} У {uname_str} нет активных варнов.')
...
async def cmd_unban(msg: Message):
    ...
    try:
        ...
        await msg.answer(f'{E_UNLOCK} {uname_str} разблокирован.')
    except TelegramBadRequest as e:
...
async def cmd_stats(msg: Message):
    ...
    await msg.answer(
        f'{E_CHART} <b>Статистика</b> — {uname_str}\n\n'
        f'{E_LOG} Логов: <b>{logs}</b> (до варна: {l2w})\n'
        f'{E_WARN} Варнов: <b>{warns}</b> (до бана: {w2b})\n'
